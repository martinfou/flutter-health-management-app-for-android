...
class SleepEnergyPage extends ConsumerStatefulWidget {
  ...
  const SleepEnergyPage({super.key, this.metricId});
...
class _SleepEnergyPageState extends ConsumerState<SleepEnergyPage> {
        ...
        (metric) {
          if (mounted) {
            setState(() {
              ...
              _sleepQuality = metric.sleepQuality?.toDouble() ?? 5.0;
              _sleepHours = metric.sleepHours ?? 7.0;
              _energyLevel = metric.energyLevel?.toDouble() ?? 5.0;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_isEditMode ? 'Edit Sleep & Energy' : 'Sleep & Energy'),
      ),
      body: _isLoading
