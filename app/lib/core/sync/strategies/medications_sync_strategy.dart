...
import 'package:health_app/features/medication_management/data/services/medications_sync_service.dart' hide SyncFailure;
...
/// Strategy for syncing medications
///
/// Wraps the existing MedicationsSyncService to implement the SyncStrategy interface.
/// Includes automatic retry with exponential backoff for transient failures.
class MedicationsSyncStrategy implements SyncStrategy {
  final MedicationsSyncService _syncService;
  final OfflineSyncQueue _offlineQueue;
  static const String _lastSyncKey = 'last_medications_sync_timestamp';
  static const String _lastSyncErrorKey = 'last_medications_sync_error';
  static const int _maxRetries = 3;
  static const Duration _retryDelay = Duration(seconds: 2);

  MedicationsSyncStrategy(this._syncService, this._offlineQueue);
  ...
  Future<Either<Failure, DataTypeSyncStatus>> _syncWithRetry(int attemptCount) async {
    try {
      ...
      final result = await _syncService.syncMedications();
      ...
      if (result.isLeft()) {
        ...
        if (failure != null) {
          ...
          if (isTransient && attemptCount < _maxRetries) {
            ...
            print('MedicationsSyncStrategy: Retry ${attemptCount + 1}/$_maxRetries after ${delayMs}ms due to: ${failure!.message}');
      ...
      if (attemptCount < _maxRetries) {
        ...
        print('MedicationsSyncStrategy: Retry ${attemptCount + 1}/$_maxRetries after ${delayMs}ms due to: $e');
        await Future.delayed(Duration(milliseconds: delayMs));
        return _syncWithRetry(attemptCount + 1);
      }

      final errorMsg = 'Medications sync error: ${e.toString()}';
  ...
  Future<void> _saveLastSyncError(String error) async {
    try {
      ...
      print('MedicationsSyncStrategy: Failed to save sync error: $e');
    }
  }
  ...
  Future<void> _clearLastSyncError() async {
    try {
      ...
      print('MedicationsSyncStrategy: Failed to clear sync error: $e');
    }
  }
  ...
  Future<Either<Failure, void>> syncItem(String operation, Map<String, dynamic> data) async {
    try {
      ...
      return Left(SyncFailure('Single medication sync failed: $e'));
    }
  }
